...
function jisikCheckForm(btn){
	...
	if (btn.form.boardTitle.value.trim().length==0){
		...
		document.jisikwriteaction.submit()
	}
}
</script>
<style>
...
<body>
	...
	<div class="section-header">
		...
		<div class="writeform">
			<form modelAttribute="boardVo" method="post" action="${pageContext.servletContext.contextPath }/community/writeaction" name="jisikwriteaction">
			<input type="hidden" name="userNo" value=${authUser.userNo }><br>	
				<input type="hidden" name="userId" value="${authUser.userId}"><br>
			 	<input type="hidden" name="boardCategory" value="질문게시판"><br>
				...
				<div class="makeTeamdiv" style="margin-left:17%;">
					<input class="makeTeam" type="button" value="작성" onclick="jisikCheckForm(this)">
				</div>
			</form>
		</div>
		<script type="text/javascript">
