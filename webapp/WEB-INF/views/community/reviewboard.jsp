...
<jsp:include page="/WEB-INF/views/includes/header.jsp" />
	...
	<div class="section-header">
          <h3 class="section-title">공모전 후기</h3>
	...
	<table class="table">	
		...
		<tr class="zxc">
	    	...
	    	<td class="title"><a href="<%=request.getContextPath() %>/community/readreview?boardNo=${reviewList.boardNo}">${reviewList.boardTitle }</a></td>
	    	<td class="text-left">${reviewList.userVo.userName }</td>
	    	<td class="text-left">${ reviewList.regDate}</td>
	    	<td class="text-left">${ reviewList.cateVo.cateName}</td>
	    	<td class="text-left">${ reviewList.hit_cnt}</td>
	    	<td class="">
	    		<c:if test="${not empty authUser}">
					<c:if test="${authUser.userNo == reviewList.userNo}">
					<button type="button" rel="tooltip" title="수정" class="btn btn-success btn-simple btn-xs"
						onclick="location.href='<%= request.getContextPath() %>/community/modify?boardNo=${reviewList.boardNo}'">
			...
			<c:if test="${not empty authUser}">
				<c:if test="${authUser.userNo == reviewList.userNo}">
					<a href="<%= request.getContextPath() %>/community/delete?boardNo=${reviewList.boardNo}" 
    ...
    <c:if test="${!empty authUser && pageContext.request.requestURI eq '/Bimanda/WEB-INF/views/community/reviewboard.jsp' }">
		<a class="makeTeam" style="margin-left:830px; font-weight: bold; font-size: 15px; color:#fff;" href="<%= request.getContextPath() %>/community/reviewwrite">글쓰기</a>
	</c:if>
    </div>
